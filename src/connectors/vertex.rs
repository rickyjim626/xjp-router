...
impl VertexConnector {
    ...
    fn auth_headers() -> Result<(Option<String>, Option<String>), ConnectorError> {
        ...
        if api_key.is_none() && access_token.is_none() {
            return Err(ConnectorError::Auth("set VERTEX_API_KEY or VERTEX_ACCESS_TOKEN".into()));
        }
...
impl Connector for VertexConnector {
    fn name(&self) -> &'static str { "vertex" }

    fn capabilities(&self) -> ConnectorCapabilities {
        ConnectorCapabilities { text: true, vision: true, video: true, tools: false, stream: false }
    }
    ...
    async fn invoke(&self, route: &EgressRoute, req: UnifiedRequest) -> Result<ConnectorResponse, ConnectorError> {
        ...
        let project = route.project.clone().or_else(|| std::env::var("VERTEX_PROJECT").ok())
            .ok_or_else(|| ConnectorError::Invalid("missing project".into()))?;
        let region = route.region.clone().or_else(|| std::env::var("VERTEX_REGION").ok())
            .ok_or_else(|| ConnectorError::Invalid("missing region".into()))?;
        let model = &route.provider_model_id;
        let url = format!("https://{}-aiplatform.googleapis.com/v1/projects/{}/locations/{}/{}:generateContent",
        ...
        if let Some(t) = req.max_output_tokens { body["generationConfig"] = json!({"maxOutputTokens": t}); }
        ...
        let chunk = UnifiedChunk { text_delta: Some(text_out), tool_call_delta: None, done: true, provider_events: Some(v) };
